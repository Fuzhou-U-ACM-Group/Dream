...
inline void add_blossom(int u,int lca,int v){
	...
	flower[b].clear();
	flower[b].pb(lca);
	for(int x=u,y;x!=lca;x=st[pa[y]]) flower[b].pb(x), flower[b].pb(y=st[match[x]]), q_push(y);
	reverse(flower[b].begin()+1,flower[b].end());
	for(int x=v,y;x!=lca;x=st[pa[y]]) flower[b].pb(x), flower[b].pb(y=st[match[x]]), q_push(y);
	set_st(b,b);
