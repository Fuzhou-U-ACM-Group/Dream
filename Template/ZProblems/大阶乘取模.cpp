...
inline int add(R int x, R int y) {
	return 0ll + x + y >= P ? 0ll + x + y - P : x + y;
}
...
void Pre() {
	...
	for (R int i = 1, d = 0; i < 131072; i <<= 1, ++d)
		fp(k, 0, i - 1) w[1][i + k] = cp(cos(Pi * k * iv[d]), sin(Pi * k * iv[d])),
...
void FFT(cp *A, int ty) {
	...
	for (R int mid = 1; mid < lim; mid <<= 1)
		for (R int j = 0; j < lim; j += (mid << 1))
			fp(k, 0, mid - 1) A[j + k + mid] = A[j + k] - (t = w[ty][mid + k] * A[j + k + mid]),
			                  A[j + k] = A[j + k] + t;
	if (!ty)
		fp(i, 0, lim - 1) A[i] = A[i] * iv[d];
}
void MTT(int *a, int *b, int len, int *c) {
	...
	FFT(p, 0), FFT(q, 0);
	fp(i, 0, lim - 1) c[i] =
	    ((((ll)(p[i].x + 0.5) % P << 16) % P << 16) + ((ll)(q[i].x + 0.5) << 16) + ((ll)(q[i].y + 0.5))) % P;
}
void calc(int *a, int *b, int n, int k) {
	...
	sum[0] = g[0];
	fp(i, 1, n + n) sum[i] = mul(sum[i - 1], g[i]);
	isum[n + n] = ksm(sum[n + n], P - 2);
	fd(i, n + n, 1) isum[i - 1] = mul(isum[i], g[i]);
	fp(i, 1, n + n) g[i] = mul(isum[i], sum[i - 1]);
	g[0] = isum[0];
	fp(i, n + 1, len - 1) f[i] = 0;
	fp(i, n + n + 1, len - 1) g[i] = 0;
	...
	sum[0] = g[0];
	fp(i, 1, n) sum[i] = mul(sum[i - 1], g[i]);
	isum[n] = ksm(sum[n], P - 2);
	fd(i, n, 1) isum[i - 1] = mul(isum[i], g[i]);
	fp(i, 1, n) g[i] = mul(isum[i], sum[i - 1]);
	g[0] = isum[0];
...
int solve(int bl) {
	...
	for (int p = 0; s >= 0; --s) {
		if (p) {
			calc(a, b, p, p + 1);
			fp(i, 0, p) a[p + i + 1] = b[i];
			a[p << 1 | 1] = 0;
			calc(a, b, p << 1, mul(p, qwq));
			p <<= 1;
			fp(i, 0, p) a[i] = mul(a[i], b[i]);
		}
		if (bl >> s & 1) {
