...
int dx2[] = {1, -1, -2, -1, 1, 2};
int dy2[] = {3, 3, 0, -3, -3, 0};
bool cane[9][22], canm[9][22], cang[9][22];
bool vish[9][22];
...
bool jiang(char g[][22], int x, int y, int nx, int ny) {
	if(nx < 0 || nx >= 9 || ny < 0 || ny >= 22) return 0;
...
bool check(char g[][22]) {
	/*
	rep(i, 0, 9) {
		rep(j, 0, 22) cout << g[i][j]; cout << endl;
	}
	cout << endl;
	*/
	rep(i, 0, 9) {
	...
	rep(i, 0, 9) rep(j, 0, 22) {
		if(g[i][j] == 'S') {
			if(jiang(g, i, j, i, j + 2)) return 1;
			if(j >= river) {
				if(jiang(g, i, j, i - 1, j + 1)) return 1;
				if(jiang(g, i, j, i + 1, j + 1)) return 1;
			}
		} else if(g[i][j] == 'H') {
			rep(d, 0, 6) {
				int x = i + dx[d], y = j + dy[d];
				if(x < 0 || x >= 9 || y < 0 || y >= 22 || g[x][y] != ' ') continue;
				rep(t, 0, 2) {
					int nd = (d + t) % 6;
					if(jiang(g, i, j, x + dx2[nd], y + dy2[nd])) return 1;
				}
			}
			rep(d, 0, 6) {
				...
				rep(t, 0, 2) {
					int nd = (d - t + 6) % 6;
					if(jiang(g, i, j, x + dx[nd], y + dy[nd])) return 1;
				}
			}
		} else if(g[i][j] == 'C') {
			rep(d, 0, 6) {
				...
				while(1) {
					x += dx[d], y += dy[d];
					if(jiang(g, i, j, x, y)) return 1;
					if(x < 0 || x >= 9 || y < 0 || y >= 22 || g[x][y] != ' ') break;
				}
			}
		} else if(g[i][j] == 'A') {
			// leap 
			...
			rep(d, 0, 6) {
				...
				while(1) {
					nx += dx[d], ny += dy[d];
					if(isl(g, nx, ny)) break;
					if(over(nx, ny)) break;
				}
				if(!iss(g, nx, ny)) continue;
				if(jiang(g, i, j, nx, ny)) return 1;
			}
			*/
			rep(d, 0, 6) {
				int x = i, y = j;
...
bool check(char g[][22], int x, int y, int nx, int ny) {
	if(nx < 0 || nx >= 9 || ny < 0 || ny >= 22) return 0;
	if(g[nx][ny] == '#' || isb(g[nx][ny])) return 0;
	char tg[9][22];
		...
		if(g[i][j] == 'S') {
			...
			rep(d, 0, 6) {
				...
				rep(t, 0, 2) {
					int nd = (d + t) % 6;
					x += dx2[nd], y += dy2[nd];
					if(over(x, y) || vish[x][y]) continue;
					if(check(g, i, j, x, y)) ++ans, vish[x][y] = 1;
				}
			}
			rep(d, 0, 6) {
				int x = i + dx2[d], y = j + dy2[d];
			...
			rep(d, 0, 6) {
				...
				if(x < 0 || x >= 9 || y < 0 || y >= 22 || g[x][y] != ' ') continue;
				x += dx2[d], y += dy2[d];
				if(x < 0 || x >= 9 || y < 0 || y >= 22 || !cane[x][y]) continue;
				ans += check(g, i, j, x, y);
			}
		} else if(g[i][j] == 'M') {
			rep(d, 0, 6) {
				int x = i + dx2[d], y = j + dy2[d];
				if(x < 0 || x >= 9 || y < 0 || y >= 22 || !canm[x][y]) continue;
				ans += check(g, i, j, x, y);
			}
		} else if(g[i][j] == 'G') {
			rep(d, 0, 6) {
