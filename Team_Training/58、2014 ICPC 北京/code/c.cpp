...
bool check(L l1, L l2) {
	...
	if(check(x, y, l1) && check(x, y, l2) && sign(fabs(l1.a.x - x) - fabs(l2.a.x - x)) == 0 && sign(fabs(l1.a.y - y) - fabs(l2.a.y - y)) == 0) {
		printf("%.1f %.1f\n", x, y);
		return 1;
	}
	return 0;
...
void solve() {
	...
	while(1) {
		...
		vis[ha] = 1;
		if(abs(a.dir - b.dir) == 2 && abs(a.x - b.x) == abs(a.y - b.y) && check0(a, b)) {
			int x = a.x + b.x;
			int y = a.y + b.y;
			printf("%.1f %.1f\n", x / 2., y / 2.);
			return ;
