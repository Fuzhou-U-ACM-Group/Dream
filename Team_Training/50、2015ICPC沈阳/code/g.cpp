...
bool solve(db v1,db v2, db t) {
	...
	db sq2 = sqrt(2);
	int cnt = 0;
	if (sign(300 * sq2 / v1 - (db) 600 / v2) <= 0) {
		...
		w[0] = x, w2[1] = 300 - x; //else w[0] = xx2, w2[1] = 300 - xx2;
		w[1] = w[2] = w[3] = 300;
		w2[0] = 0;  w2[2] = w2[3] = 300;
		rep(i, 1, 4) {
			w2[i] += w2[i-1];
			w[i] += w[i-1];
		...
		if (check(xx1)) { x = min(x, xx1);}
		if (check(xx2)) { x = min(x, xx2);}
		w[0] = sqrt(x * x + 300 * 300); 
		w2[2] = 300 - x;
		w[1] = w[2] = w[3] = 300;
		w2[0] = w2[1] = 0, w2[3] = 300;
		rep(i, 1, 4) {
			w[i] += w[i-1];
			w2[i] += w2[i-1];
