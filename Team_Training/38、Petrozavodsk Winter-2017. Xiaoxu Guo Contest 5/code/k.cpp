...
int now, ans, pw2[N], s[N], k[N], n, m, ma, vis[N], b[N];
vi tmp, tmp2, a[N];
int p, cnt[N], pos[N];
...
int main() {
  ...
  std::cin.tie(0);
  pw2[0] = 1; rep(i, 1, 200001) pw2[i] = 1ll * pw2[i-1] * 2 % P;
  while (cin >> n >> m) {
    tmp.clear(); ans = now = 0; ma = 0;
    rep(i, 1, n+1) {
      a[i].clear();
      int x;
      cin >> k[i];
    ...
    fw.init(ma);
    rep(i, 1, ma+1) now = add(now, pw2[cnt[i]] - 1), fw.upd(i, pw2[cnt[i]] - 1);
    now = add(now, 1);
    ans = now;
    rep(i, 1, m+1) {
